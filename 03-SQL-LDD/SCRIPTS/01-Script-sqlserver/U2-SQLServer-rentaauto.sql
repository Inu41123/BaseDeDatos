...
CREATE TABLE Sucursal (
    ...
    CONSTRAINT CHK_Sucursal_numsucursal_Positivo 
    CHECK (numsucursal > 0),
    CONSTRAINT UQ_Sucursal_Nombre 
    UNIQUE (Nombre),
    CONSTRAINT UQ_Sucursal_ubicacion 
    UNIQUE (ubicacion),
...
CREATE TABLE Cliente (
    ...
    CONSTRAINT CHK_Cliente_Numcliente_Positivo 
    CHECK (Numcliente > 0),
    CONSTRAINT UQ_Cliente_Curb 
    UNIQUE (Curb),
    CONSTRAINT CHK_Cliente_Numero_Positivo 
    CHECK (Numero > 0),
...
CREATE TABLE Vehiculo (
    ...
    CONSTRAINT CHK_Vehiculo_Numdevehiculo_Positivo 
    CHECK (Numdevehiculo > 0),
    CONSTRAINT UQ_Vehiculo_Placa 
    UNIQUE (Placa),
    CONSTRAINT CHK_Vehiculo_Anio_Valido 
    CHECK (Anio > 1900),
    CONSTRAINT CHK_Vehiculo_NumclienteFK_Positivo 
    CHECK (NumclienteFK > 0),
    CONSTRAINT CHK_Vehiculo_NumsucursalFK_Positivo 
    CHECK (NumsucursalFK > 0),
    CONSTRAINT PK_Vehiculo 
    PRIMARY KEY (Numdevehiculo),
    CONSTRAINT FK_Vehiculo_Cliente 
    FOREIGN KEY (NumclienteFK) 
    REFERENCES Cliente(Numcliente),
    CONSTRAINT FK_Vehiculo_Sucursal 
    FOREIGN KEY (NumsucursalFK) 
